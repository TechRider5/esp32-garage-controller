...
#include "helpers/controls/LedController.h"
#include "helpers/controls/DoorController.h"
...
// door pulsing handled by DoorController; LED pulse handled by LedController
...
void loop() {
  ...
  // Handle LED pulse state machine
  tickPulse(nowMs);
  if (consumePulseFinishedFlag()) {
    updateStatus("off");
  }
    ...
    if (code == 200) {
      ...
      if (!deserializeJson(doc, payload)) {
        ...
        if (doorCmd != lastDoorCmd) {
          if (doorCmd == "door1")      triggerDoor1();
          else if (doorCmd == "door2") triggerDoor2();
